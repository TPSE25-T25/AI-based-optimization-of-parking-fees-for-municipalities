...
class TestDriverDecision:
    ...
    def test_select_parking_lot_affordable(self, decision, test_driver, lot1, lot2):
        """Test that driver selects affordable lot."""
        lots = [lot1, lot2]
        selected = decision.select_parking_lot(test_driver, lots)
        assert selected is not None
        assert selected in lots
    
    def test_select_parking_lot_unaffordable(self, decision, test_driver):
        ...
        result = decision.select_parking_lot(test_driver, [expensive_lot])
        assert result is None
    
    def test_select_parking_lot_empty_list(self, decision, test_driver):
        """Test behavior with no available lots."""
        result = decision.select_parking_lot(test_driver, [])
        assert result is None
...
def test_city():
    ...
    lot2 = ParkingLot(
        ...
        pseudonym="Lot2",
        price=Decimal('5.00'),
        position=(520.0, 520.0),
        maximum_capacity=50,
        current_capacity=0
    )
    
    city.add_parking_lot(lot1)
    city.add_parking_lot(lot2)
...
class TestParkingSimulation:
    ...
    def test_simulation_resets_capacity(self, simulation, test_city, test_drivers):
        ...
        test_city.parking_lots[0].current_capacity = 50
        ...
        total_capacity = sum(lot.maximum_capacity for lot in test_city.parking_lots)
...
def batch_city():
    ...
    poi = PointOfInterest(id=1, pseudonym="Downtown", position=(500.0, 500.0))
    city.add_point_of_interest(poi)
