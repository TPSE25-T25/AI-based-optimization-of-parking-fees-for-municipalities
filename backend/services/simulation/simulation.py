...
from ...models.city import City, ParkingLot
...
class DriverDecision:
    ...
    def calculate_lot_score(
        ...
        Args:
            ...
            parking_lot: The parking lot being evaluated
    ...
    def select_parking_lot(
        self,
        driver: Driver,
        available_lots: List[ParkingLot]
    ) -> Optional[ParkingLot]:
        """
...
class ParkingSimulation:
    ...
    def run_simulation(
        ...
        if reset_capacity:
            for lot in city_copy.parking_lots:
        ...
        parked_count = 0
        rejected_count = 0
        lot_revenues: Dict[int, Decimal] = {lot.id: Decimal(0) for lot in city_copy.parking_lots}
        
        # Simulate each driver
        for driver in drivers:
            # Get available lots
            available_lots = [lot for lot in city_copy.parking_lots if not lot.is_full()]
            
            # Driver selects best lot
            selected_lot = self.decision_maker.select_parking_lot(driver, available_lots)
        ...
        # Occupancy metrics
        occupancy_rates = [lot.occupancy_rate() for lot in city_copy.parking_lots]
        ...
        lot_occupancy_rates = {lot.id: lot.occupancy_rate() for lot in city_copy.parking_lots}
        
        # Construct metrics object
        metrics = SimulationMetrics(
            total_revenue=total_revenue,
            average_revenue_per_lot=total_revenue / len(city_copy.parking_lots) if city_copy.parking_lots else Decimal(0),
    ...
    def evaluate_price_configuration(
        ...
        city_copy = deepcopy(city)
        sorted_lots = sorted(city_copy.parking_lots, key=lambda x: x.id)
